...
- **双向分片加载**：首次打开和远距离跳转都只加载目标楼层附近窗口；连续阅读时再通过顶部/底部哨兵补齐前后楼层，向上插入时保持当前视口不跳动。
- **滑动窗口控 DOM**：评论区最多保留 `80` 条已渲染楼层，超过后自动裁剪离屏内容，长帖滚动时不会无限膨胀 DOM。
- **流畅滚动优化**：时间轴使用常量级当前楼层探测；跨很远的跳转直接定位到目标窗口，不再把中间所有楼层逐批插入后再滚动过去。
...
const MAX_RENDERED_POSTS = 80; // 评论区最多保留的已渲染楼层数
...
- **数据获取与定位**：`GET /t/{id}.json` 取得话题信息和完整 `post_stream.stream`；续读、通知直达、时间轴点击和“跳到最新”都会先根据目标楼层确定流索引，再通过 `GET /t/{id}/posts.json?post_ids[]=...` 只加载目标附近窗口。
- **双向续加载**：目标窗口上下各维护一个游标，顶部/底部哨兵进入视口时分别加载前一批或后一批；向上加载根据内容高度变化补偿滚动位置。
- **滑动窗口裁剪**：连续阅读超过 `MAX_RENDERED_POSTS` 后，会自动裁剪最早或最晚一批已渲染楼层，同时保留内存缓存和楼中楼缓存，需要时可无网络重新渲染。
- **请求队列**：所有只读 GET 请求串行执行且保持最小间隔；HTTP 429 最多重试 3 次，优先采用服务端 `Retry-After`，否则按 500/1000/2000ms 退避。写操作不自动重试。
- **楼中楼**：以楼层号建立映射，按 `reply_to_post_number` 挂到父节点；跨分块未就绪的父级先暂存，块加载完后回扫归位，已加载过的楼中楼响应会在窗口切换后复用。
